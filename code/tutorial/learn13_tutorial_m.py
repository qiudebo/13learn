...
if __name__ == '__main__':
    ...
    url = 'http://www.tutorialspoint.com/redis/'
    url = 'http://www.tutorialspoint.com/ionic/'
    # 文件名
    filename = 'ionic'
    ...
    try:
        ...
        # #方式1：
        districtInfos = soup.findAll('ul', attrs={'class': 'nav nav-list primary left-menu'})
        #districtInfos = soup.findAll('ul', attrs={'class': 'nav nav-list primary left-menu special'})
        count =0 ;
        for subinfos in districtInfos:
            links = subinfos.select('li > a')
            a = []
            b = []
            for link in links:
            ...
            print s
            s.to_csv(filename+str(count)+'.csv')
