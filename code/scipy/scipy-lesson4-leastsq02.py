...
#y0 = [27.7,26.8,26.5,26.3,26.1,25.7,25.3,24.8] # 存在异常值、离群点
...
# 建立y与t的经验公式 y=f(t)
# 从图形可以看出，这些点大致接近一条直线(拟合指数函数)，所以可以认为 y=f(t)是线性函数，设y=k*e**mt；
# 求k、m，
# 分析过程：找出一条实验值和函数值(经验值、估计值)偏差都很小的直线,目标函数 偏差的和很小。
# 得出目标函数：这种根据偏差平方和为最小的条件来选择常数k、m的方法叫做最小二乘法。
# 对目标函数求极值
# 对y、t求偏导数，得到k、m，得到线性方程
...
r = optimize.leastsq(residuals,[1,0]) # k,m初始值，可随意指定，随着迭代次数的增加，m,k
k,m=r[0]
print k,m
...
# 坐标信息
ax.set_ylim(5,60) # 坐标范围

ax.set_title(u'分子化学反应速度速度')
ax.set_xlabel('t')
ax.set_ylabel('y')
ax.grid(True)
ax.legend() # 图例
